...
from pydantic import NonNegtiveInt
from src.models.image_filters import FilterType
def compute_structual_tensor(image: np.ndarray, window_radius: NonNegtiveInt, filter_type: Optional[FilterType]):

    five_point_differentiation = np.array([1, -8, 0, 8, -1])
    intensity_1 = convolve(image, five_point_differentiation[np.newaxis, np.newaxis, :], mode='constant')
    intensity_2 = convolve(image, five_point_differentiation[np.newaxis, :, np.newaxis], mode='constant')
    intensity_3 = convolve(image, five_point_differentiation[:, np.newaxis, np.newaxis], mode='constant')
    ...
    return {'S11': convolve(structure_tensor_11, weights, mode='constant'),
